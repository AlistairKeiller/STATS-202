...
```{python}
g = sns.PairGrid(weekly, hue="Direction")
...
```{python}
so.Plot(weekly, color="Direction").pair(x=['Year', 'Lag1', 'Lag2', 'Lag3', 'Lag4', 'Lag5', 'Volume', 'Today'], y=['Year', 'Lag1', 'Lag2', 'Lag3', 'Lag4', 'Lag5', 'Volume', 'Today']).add(so.Dot())
```
...
```{python}
weekly_bool = weekly.copy()
weekly_bool["Direction"] = (weekly["Direction"] == "Up").astype(int)
smf.logit('Direction ~ Volume + Lag1 + Lag2 + Lag3 + Lag4 + Lag5', data=weekly_bool).fit().summary()
```
